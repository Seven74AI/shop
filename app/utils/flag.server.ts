...
function matchesAudience(
	...
	if (audience.userIds?.length && ctx.userId) {
		if (audience.userIds.includes(ctx.userId)) return true
	}
	if (audience.countries?.length && ctx.country) {
		if (audience.countries.includes(ctx.country)) return true
	}
	if (audience.roles?.length && ctx.roles?.length) {
		for (const role of ctx.roles) {
			if (audience.roles.includes(role)) return true
		}
	}
	// If audience has filters but context doesn't match any → false
	if (
		(audience.userIds?.length || audience.countries?.length || audience.roles?.length) &&
		!ctx.userId &&
		!ctx.country &&
		!ctx.roles?.length
	) {
...
export async function isFlagEnabled(
	...
	if (flag.rolloutPercentage != null && flag.rolloutPercentage > 0 && flag.rolloutPercentage <= 100) {
		...
		// No userId for rollout → fall back to enabled (if no rollout gate)
		if (flag.audience) return false // audience didn't match, no userId → false
	}
