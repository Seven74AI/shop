...
export async function loader({ params, request }: Route.LoaderArgs) {
	...
	const breadcrumbItems = [{ name: 'Home', href: `${siteUrl}/` }]
	if (category.parent) {
		breadcrumbItems.push({
			name: category.parent.name,
			href: `${siteUrl}/shop/categories/${category.parent.slug}`,
		})
	}
	breadcrumbItems.push({
		name: category.name,
		href: `${siteUrl}/shop/categories/${category.slug}`,
	})

	const breadcrumbLd = buildBreadcrumbListLd(breadcrumbItems)
				...
				<div>
					...
					<p className="text-muted-foreground mt-1">
						...
						{filteredProducts.length === 1
							? t('shop.products.product_one')
							: t('shop.products.product_other')}
					</p>
				</div>
